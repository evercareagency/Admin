...
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"ace"})', ctx), 'office', 'unknown source defaults to office');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"local"})', ctx), 'office', 'phone-session rows are office intake');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"office"})', ctx), 'office', 'exact office');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"Office"})', ctx), 'office', 'unknown capitalization defaults to office');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:" aide "})', ctx), 'aide', 'trimmed exact aide');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"aide"})', ctx), 'aide', 'exact aide');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"Aide"})', ctx), 'office', 'Aide is not the exact value');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"caregiver"})', ctx), 'office', 'caregiver is not a source value');
assert.strictEqual(vm.runInContext('coverIntakeSource({source:"aide_record_call_off"})', ctx), 'office', 'callable name is not a source value');
assert.strictEqual(vm.runInContext('coverIntakeSource({calloff_source:"aide"})', ctx), 'office', 'calloff_source is ignored');
assert.strictEqual(vm.runInContext('coverIntakeSource({submitted_by:"aide"})', ctx), 'office', 'submitted_by is ignored');
assert.strictEqual(vm.runInContext('coverIntakeSource({aide_submitted:true})', ctx), 'office', 'aide_submitted is ignored');
assert.strictEqual(vm.runInContext('coverIntakeSource({from_aide:true})', ctx), 'office', 'from_aide is ignored');
assert.strictEqual(vm.runInContext('coverIntakeSource({submitted_by_aide:true})', ctx), 'office', 'submitted_by_aide is ignored');
assert.strictEqual(vm.runInContext('coverIntakeSource({aide_call_off:true, source:"aide"})', ctx), 'aide', 'only source aide counts');
assert.strictEqual(vm.runInContext('coverIntakeSource({regular_aide:{name:"Bea"}})', ctx), 'office', 'the regular aide object is not source');
assert.strictEqual(vm.runInContext('coverIntakeSource({intakeSource:"aide"})', ctx), 'office', 'a stored chip value is not the list source');
const intakeFn = extractFn(html, 'function coverIntakeSource(src)');
assert.ok(!/aide_submitted|from_aide|calloff_source|caregiver|submitted_by/.test(intakeFn), 'no alternate flag fallbacks');
